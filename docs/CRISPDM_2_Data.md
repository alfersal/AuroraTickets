...
Por un lado, tengo los **datos de Clickstream** (el rastro que deja el usuario). Esto me llega en formato JSON Lines, generado tanto por el front como por el backend de la web. Aquí viene información como la IP, el session\_id, en qué momento exacto (timestamp) hace clic y a qué endpoint (URL) va.
...
* `event_type`: Me dice si el usuario hizo un *view* (vista), un *click*, o un *begin\_checkout*.  
